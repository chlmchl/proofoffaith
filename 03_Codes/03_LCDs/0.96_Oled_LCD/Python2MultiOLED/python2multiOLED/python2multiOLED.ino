...
void sendToDisplay(const char* input_data) {
  int rc;
  int rc2;
  int rc3;
  int rc4;
  ...
  rc = obdI2CInit(&obd[0], MY_OLED1, OLED_ADDR, FLIP180, INVERT, 0, LCD1_SDA_PIN, LCD1_SCL_PIN, RESET_PIN, 400000L); // use standard I2C bus at 400Khz
    if (rc != OLED_NOT_FOUND)
    {
      ...
      obdWriteString(&obd[0], 0,0,0, (char*)input_data, FONT_6x8, 0, 1);
    }
  ...
  rc3 = obdI2CInit(&obd[2], MY_OLED3, OLED_ADDR, FLIP180, INVERT, 0, LCD3_SDA_PIN, LCD3_SCL_PIN, RESET_PIN, 400000L); // use standard I2C bus at 400Khz
    if (rc3 != OLED_NOT_FOUND)
    {
      ...
      obdWriteString(&obd[2], 0,0,0,(char *)"Burp", FONT_6x8, 0, 1);
    }
...
{ 
  ...
  if(Serial.available()){ 
      String str = Serial.readString();   // receive string from python
      const char *ch = str.c_str();       // convert string to const char
      Serial.println(ch);
      delay(1000);                        // somehow it's better to be sure the whole string was received
      sendToDisplay(ch);                  // send the converted string to displays
  }
  //delay(100); 
} /* loop() */
